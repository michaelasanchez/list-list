...
import type { GoogleAuthCodeResponse, UserProfile } from '../auth';
...
export const useAuth = (clientId: string): AuthState => {
  ...
  useEffect(() => {
    ...
    const checkSession = async () => {
      try {
        const profile = await new UserApi().Me();
        setUser(profile);
      } catch {
        setUser(null);
      } finally {
  ...
  const handleLogin = async (code: string) => {
    setLoading(true);
    try {
      const profile = await new UserApi().Login(code);
      setUser(profile);
    } catch (err) {
