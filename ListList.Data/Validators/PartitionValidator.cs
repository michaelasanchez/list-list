...
public class PartitionValidator(IListListContext _context) : IPartitionValidator
{
    ...
    {
        var isDeleted = Guid.TryParse(token, out var partitionId)
    ...
    public async Task IsValidIndex(Guid? userId, int? index, ValidationResult result)
    {
        var isValidIndex = index >= 0 &&
            index <= await _context.Partitions
    ...
    {
        var tokenExists = Guid.TryParse(token, out var partitionId)
            ? await _context.Partitions
                .Include(z => z.ShareLinks)
                .AnyAsync(z =>
                    z.Id == partitionId ||
    ...
    public async Task IsOwnedByUser(Guid? userId, string token, ValidationResult result)
    {
        var userHasAccess = !userId.HasValue
            ? false
            : Guid.TryParse(token, out var partitionId)
                ? await _context.Partitions
                    .AnyAsync(z =>
                        z.Id == partitionId &&
                        z.OwnerId == userId)
